...
AirTemperatureDeratingParser::AirTemperatureDeratingParser(string & filename, map<string, AirTemperatureDeratingTable> & tables) {
	...
	for (vector<string>::iterator it = categories.begin(); it != categories.end(); it++) {
		tables.insert(make_pair(*it, AirTemperatureDeratingTable(map.at(*it))));
	}
}
